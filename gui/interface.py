...
class AutoPressUI:
    ...
    def setup_ui(self):
        self.root.title("AutoPress Pro")
        self.root.geometry("450x300")

        main_frame = ttk.Frame(self.root, padding=15)
        main_frame.pack(fill=tk.BOTH, expand=True)

        # Поле для названия окна
        ...
        # Поле для клавиши с подсказкой
        ttk.Label(main_frame, text="Клавиша для нажатия:").grid(row=1, column=0, sticky=tk.W, pady=5)
        ...
        # Поле для интервала
        ttk.Label(main_frame, text="Интервал (секунды):").grid(row=2, column=0, sticky=tk.W, pady=5)
        ...
        # Кнопка управления
        self.control_btn = ttk.Button(main_frame, text="Старт", command=self.toggle_auto_press)
    ...
    def toggle_auto_press(self):
        if self.controller.is_running:
            ...
            self.control_btn.config(text="Старт")
            self.status_var.set("Автонажатие остановлено")
        else:
            params = {
            ...
            if errors:
                ...
                self.control_btn.config(text="Стоп")
                self.status_var.set(f"Автонажатие {params['key'].upper()}...")
